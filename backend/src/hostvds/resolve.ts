 ...
 */
export async function resolveFlavor(nameOrUuid: string): Promise<string> {
  ...
  if (looksLikeUuid(key)) {
    ...
    throw new Error(`Flavor UUID ${key} not found in active HostVDS region`);
  }
  const byName = list.find((f) => f.name === key);
  if (byName) return byName.id;
  throw new Error(
    `Flavor "${key}" not found in region. Available: ${list
      .map((f) => f.name)
      .slice(0, 20)
      .join(", ")}`,
  );
...
export async function resolveImage(nameOrUuid: string): Promise<string> {
  const key = nameOrUuid.trim();
  const data = await hostVdsRequest<{
    images: Array<{ id: string; name: string }>;
  }>("/v2/images?limit=200", { service: "image" });
  const list = Array.isArray(data.images) ? data.images : [];
  if (looksLikeUuid(key)) {
    const byId = list.find((i) => i.id === key);
    if (byId) return byId.id;
    throw new Error(`Image UUID ${key} not found in active HostVDS region`);
  }
  const byName = list.find((i) => i.name === key);
  if (byName) return byName.id;
  throw new Error(
    `Image "${key}" not found. Available: ${list
      .map((i) => i.name)
      .slice(0, 20)
      .join(", ")}`,
  );
...
export async function resolveNetwork(nameOrUuid: string): Promise<string> {
  const key = nameOrUuid.trim();
  const data = await hostVdsRequest<{
    networks: Array<{ id: string; name: string }>;
  }>("/v2.0/networks", { service: "network" });
  const list = Array.isArray(data.networks) ? data.networks : [];
  if (looksLikeUuid(key)) {
    const byId = list.find((n) => n.id === key);
    if (byId) return byId.id;
    throw new Error(`Network UUID ${key} not found in active HostVDS region`);
  }
  const byName = list.find((n) => n.name === key);
  if (byName) return byName.id;
  throw new Error(
    `Network "${key}" not found. Available: ${list
      .map((n) => n.name)
      .slice(0, 20)
      .join(", ")}`,
  );
...
export async function assertSecurityGroupExists(name: string): Promise<void> {
  const data = await hostVdsRequest<{
    security_groups: Array<{ id: string; name: string }>;
  }>("/v2.0/security-groups", { service: "network" });
  const list = Array.isArray(data.security_groups) ? data.security_groups : [];
  if (!list.some((sg) => sg.name === name)) {
    throw new Error(
      `Security group "${name}" not found. Available: ${list.map((s) => s.name).join(", ")}`,
    );
  }
}
