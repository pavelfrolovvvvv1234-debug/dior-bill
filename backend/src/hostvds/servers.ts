import { hostVdsRequest, HostVdsApiError } from "./client";
 ...
 */
export async function hostVdsCreateServer(input: HostVdsCreateServerInput): Promise<HostVdsServer> {
...
export async function hostVdsGetServer(serverId: string): Promise<HostVdsServer> {
...
export async function hostVdsFindServerByVpsId(vpsId: string): Promise<HostVdsServer | null> {
  try {
...
export async function hostVdsDeleteServer(serverId: string): Promise<void> {
  try {
    await hostVdsRequest(`/servers/${encodeURIComponent(serverId)}`, { method: "DELETE" });
  } catch (err) {
...
export async function hostVdsStartServer(serverId: string): Promise<void> {
  await hostVdsRequest(`/servers/${encodeURIComponent(serverId)}/action`, {
    method: "POST",
    body: { "os-start": null },
  });
...
export async function hostVdsStopServer(serverId: string): Promise<void> {
  await hostVdsRequest(`/servers/${encodeURIComponent(serverId)}/action`, {
    method: "POST",
    body: { "os-stop": null },
  });
...
export async function hostVdsChangePassword(serverId: string, adminPass: string): Promise<void> {
  await hostVdsRequest(`/servers/${encodeURIComponent(serverId)}/action`, {
    method: "POST",
...
export async function hostVdsWaitForServer(
  ...
  while (Date.now() < deadline) {
    last = await hostVdsGetServer(serverId);
