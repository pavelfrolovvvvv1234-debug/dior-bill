    ...
    <>
      ...
      <PageContainer>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <KpiCard label="MRR" value={formatMoney(kpis.mrr)} icon={DollarSign} href="/analytics" />
          <KpiCard
            label="Revenue (month)"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard
            ...
            href="/referrals"
          />
          <KpiCard label="Open tickets" value={String(kpis.openTickets)} icon={Ticket} href="/support" />
        </div>

        <div className="grid gap-6 lg:grid-cols-2">
          <Panel title="Recent users" description="Latest registrations" action={<Link href="/users" className="text-xs text-primary">View all</Link>}>
            <ul className="space-y-3 text-sm">
              {data.recentUsers.map((u) => (
                <li key={u.id} className="flex items-center justify-between gap-2">
                  <Link href={`/users/${u.id}`} className="truncate hover:text-primary">
          ...
          <Panel title="Open tickets" description="Needs staff attention" action={<Link href="/support" className="text-xs text-primary">Inbox</Link>}>
            <ul className="space-y-3 text-sm">
              {data.recentTickets.map((t) => (
                <li key={t.id} className="flex items-center justify-between gap-2">
                  <Link href={`/support/${t.id}`} className="truncate hover:text-primary">
                    {t.subject}
                  </Link>
