...
const STORAGE_KEY = "dior:support:contacts-banner:v1";

/**
 * Same visual pattern as DashboardContactsBanner — Telegram CTA on tickets pages.
 * Dismiss persists in localStorage; exit uses the same motion as the dashboard strip.
 */
export function SupportContactsBanner() {
  const { t } = useI18n();
      ...
      >
        ...
        <div className="min-w-0 flex-1 space-y-1">
          <p className="text-sm font-medium tracking-tight text-foreground">
            {t("support.contactsBanner.title")}
          </p>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {t("support.contactsBanner.body")}{" "}
            <a
        ...
        <button
          ...
          aria-label={t("support.contactsBanner.dismiss")}
        >
